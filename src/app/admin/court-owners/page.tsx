...
import { Alert, Avatar, Button, Chip, EmptyState, Input, Label, Link, ListBox, Modal, Select, Spinner, Table, TextField } from "@heroui/react";
...
const STATUS_OPTIONS = [{ key: "", label: "Tất cả" }, { key: "Draft", label: "Chưa gửi" }, { key: "Pending", label: "Chờ xác minh" }, { key: "Approved", label: "Đã xác minh" }, { key: "Rejected", label: "Bị từ chối" }];
...
function CourtOwnersContent() {
  ...
  async function update(verificationStatus: 1 | 2) { if (!selected) return; if (verificationStatus === 2 && !reason.trim()) return; setPending(true); setError(null); try { await updateOwnerVerification(selected.id, { verificationStatus, rejectionReason: verificationStatus === 2 ? reason.trim() : undefined }); setSelected(null); await load(); } catch { /* apiFetch shows the toast. */ } finally { setPending(false); } }
  ...
  return <div className="space-y-5">
    ...
    <Modal isOpen={Boolean(selected)} onOpenChange={(open) => { if (!open) setSelected(null); }}><Modal.Backdrop><Modal.Container><Modal.Dialog className="sm:max-w-xl"><Modal.CloseTrigger /><Modal.Header><Modal.Heading>Hồ sơ chủ sân</Modal.Heading></Modal.Header><Modal.Body>{selected && <div className="space-y-4"><div className="grid gap-3 text-sm sm:grid-cols-2"><Info label="Họ tên" value={selected.fullName} /><Info label="Email" value={selected.email} /><Info label="Điện thoại" value={selected.phone} /><Info label="Doanh nghiệp" value={selected.businessName} /><Info label="Giấy phép kinh doanh" value={selected.businessLicenseNo} /><Info label="Mã số thuế" value={selected.taxCode} /><Info label="Địa chỉ kinh doanh" value={selected.businessAddress} /><Info label="Ngày gửi" value={selected.submittedAt ? formatDate(selected.submittedAt) : null} /></div>{selected.businessLicenseDocumentUrl && <Link href={selected.businessLicenseDocumentUrl} target="_blank" rel="noreferrer" className="inline-flex text-sm">Xem ảnh giấy phép <ArrowUpRightFromSquare className="size-4" /></Link>}{selected.rejectionReason && <Alert status="warning"><Alert.Indicator /><Alert.Content><Alert.Description>Lý do từ chối trước đó: {selected.rejectionReason}</Alert.Description></Alert.Content></Alert>}{rejecting && <TextField value={reason} onChange={(value) => setReason(String(value))} isRequired><Label>Lý do từ chối</Label><Input placeholder="Nêu lý do để chủ sân có thể bổ sung hồ sơ" /></TextField>}</div>}</Modal.Body><Modal.Footer><Button variant="ghost" onPress={() => setSelected(null)} isDisabled={pending}>Đóng</Button>{selected?.verificationStatus === "Pending" && <Button onPress={() => update(1)} isDisabled={pending}>{pending && <Spinner size="sm" />}Duyệt</Button>}{selected?.verificationStatus === "Pending" && (!rejecting ? <Button variant="danger" onPress={() => setRejecting(true)} isDisabled={pending}>Từ chối</Button> : <Button variant="danger" onPress={() => update(2)} isDisabled={pending || !reason.trim()}>{pending && <Spinner size="sm" />}Xác nhận từ chối</Button>)}</Modal.Footer></Modal.Dialog></Modal.Container></Modal.Backdrop></Modal>
  </div>;
}
