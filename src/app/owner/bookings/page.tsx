...
export default function OwnerBookingsPage() { return <OwnerGuard><OwnerShell activeItem="bookings"><BookingsContent /></OwnerShell></OwnerGuard>; }
...
function BookingsContent() {
  ...
  const [status, setStatus] = useState(""); const [search, setSearch] = useState(""); const [loading, setLoading] = useState(true); const [bookingLoading, setBookingLoading] = useState(false); const [error, setError] = useState<string | null>(null); const [selected, setSelected] = useState<BookingResponseDto | null>(null);
  ...
  return <div className="mx-auto max-w-[1440px] space-y-6"><OwnerPageHeader title="Đơn đặt sân" description="Theo dõi và xử lý đơn theo từng cơ sở" />
    ...
    <BookingDrawer booking={selected} onClose={() => setSelected(null)} onChanged={loadBookings} />
  </div>;
}
...
function BookingDrawer({ booking, onClose, onChanged }: { booking: BookingResponseDto | null; onClose: () => void; onChanged: () => Promise<void> }) {
  ...
  return <Drawer isOpen onOpenChange={(open) => { if (!open) onClose(); }}><Drawer.Backdrop><Drawer.Content placement="right" className="w-full sm:max-w-2xl"><Drawer.Dialog><Drawer.Header><Drawer.Heading>Chi tiết đơn #{booking.id}</Drawer.Heading><Drawer.CloseTrigger /></Drawer.Header><Drawer.Body className="space-y-5">
