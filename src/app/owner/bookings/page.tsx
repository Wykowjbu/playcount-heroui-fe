...
import { Alert, AlertDialog, Button, Card, Drawer, Label, ListBox, SearchField, Select, Skeleton, TextArea, TextField, Table } from "@heroui/react";
...
function BookingDrawer({ booking, onClose, onChanged }: { booking: BookingResponseDto | null; onClose: () => void; onChanged: () => Promise<void> }) {
  ...
  useEffect(() => {
    if (!booking) return;
    let active = true;
    setReason(""); setPayments([]); setPaymentError(""); setPaymentLoading(true);
