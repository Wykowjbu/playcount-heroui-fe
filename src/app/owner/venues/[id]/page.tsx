...
function CourtsTab({ venueId, courts, onRefresh }: { venueId: number; courts: CourtDto[]; onRefresh: () => Promise<void> }) {
  ...
  async function removeCourt() {
    ...
    catch (removeError) { setError(removeError instanceof Error ? removeError.message : "Không thể xóa sân"); }
...
function OpeningHoursTab({ venueId, hours, onRefresh }: { venueId: number; hours: OpeningHourDto[]; onRefresh: () => Promise<void> }) {
  ...
  async function save() {
    ...
    catch (saveError) { setError(saveError instanceof Error ? saveError.message : "Không thể lưu giờ mở cửa"); }
...
function AmenitiesTab({ active, venue, onRefresh }: { active: boolean; venue: VenueResponseDto; onRefresh: () => Promise<void> }) {
  ...
  async function save() {
    ...
    catch (saveError) { setError(saveError instanceof Error ? saveError.message : "Không thể lưu tiện ích"); }
    finally { setSaving(false); }
  }
...
function ImagesTab({ venue, onRefresh }: { venue: VenueResponseDto; onRefresh: () => Promise<void> }) {
  ...
  const [error, setError] = useState("");
  async function addImage() { if (!file) return; const validation = validateImageFile(file); if (validation) { setError(validation); return; } setPending(true); setError(""); try { const { url } = await uploadFile(file, "venues"); await addVenueImage(venue.id, url); await onRefresh(); setModalOpen(false); setFile(null); if (inputRef.current) inputRef.current.value = ""; } catch (uploadError) { setError(uploadError instanceof Error ? uploadError.message : "Không thể thêm hình ảnh"); } finally { setPending(false); } }
  async function removeImage() { if (imageToDelete == null) return; setPending(true); try { await deleteVenueImage(venue.id, imageToDelete); setImageToDelete(null); await onRefresh(); } catch (removeError) { setError(removeError instanceof Error ? removeError.message : "Không thể xóa hình ảnh"); } finally { setPending(false); } }
  async function cover(imageId: number) { setPending(true); try { await setCoverImage(venue.id, imageId); await onRefresh(); } catch (coverError) { setError(coverError instanceof Error ? coverError.message : "Không thể đặt ảnh bìa"); } finally { setPending(false); } }
...
function StaffTab({ active, venueId }: { active: boolean; venueId: number }) {
  ...
  async function add(event: React.FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); setPending(true); setError(""); try { await addStaff(venueId, { email: String(data.get("email")), role: String(data.get("role")) }); await load(); setModalOpen(false); } catch (addError) { setError(addError instanceof Error ? addError.message : "Không thể thêm nhân viên"); } finally { setPending(false); } }
  async function remove() { if (!staffToRemove) return; setPending(true); try { await removeStaff(venueId, staffToRemove.id); setStaffToRemove(null); await load(); } catch (removeError) { setError(removeError instanceof Error ? removeError.message : "Không thể gỡ nhân viên"); } finally { setPending(false); } }
